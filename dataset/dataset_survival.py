...
class Generic_MIL_Survival_Dataset(Generic_WSI_Survival_Dataset):
    ...
    def __getitem__(self, idx):
        ...
        grade = self.slide_data['grade'][idx]
        ...
        if not self.use_h5:
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    return (path_features, torch.zeros((1,1)), label, event_time, c, grade)
                    ...
                    genomic_features = torch.tensor(self.genomic_features.iloc[idx])
                    return (torch.zeros((1,1)), genomic_features, label, event_time, c, grade)
                    ...
                    return (path_features, genomic_features, label, event_time, c, grade)

                elif self.mode == 'coattn':
                    for slide_id in slide_ids:
                        wsi_path = os.path.join(data_dir, 'pt_files', '{}.pt'.format(slide_id.rstrip('.svs')))
                        tcga-gbmlgg
                        wsi_path = os.path.join(data_dir, 'pt_files1', '{}.pt'.format(slide_id.rstrip('.svs')))
                    ...
                    return (path_features, omic1, omic2, omic3, omic4, omic5, omic6, label, event_time, c, grade)
                else:
