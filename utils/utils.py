...
def collate_MIL_survival(batch):
    ...
    grade=torch.LongTensor([item[5] for item in batch])
    return [img, omic, label, event_time, c, grade]
...
def collate_MIL_survival_sig(batch):
    ...
    grade = torch.LongTensor([item[10] for item in batch])
    return [img, omic1, omic2, omic3, omic4, omic5, omic6, label, event_time, c, grade]
...
def get_custom_exp_code(args):
    ...
    ### Model Type
    #if args.model_type == 'max_net': #old version
