...
parser = argparse.ArgumentParser(
    description='Configurations for Survival Analysis and Grade Classification on TCGA Data.')
...
### Model Parameters.
parser.add_argument('--model_type',      type=str, choices=['snn', 'amil', 'mcat', 'motcat', 'mome', 'dynmome'], 
...
parser.add_argument('--dynmome_gating_network', type=str, choices=['MLP', 'CNN', 'CosMLP', 'None'], default='CosMLP',
                    help='Type of gating network structure (Default: CosMLP)')
parser.add_argument('--dynmome_expert_idx', type=int, default=0,
                    help='Expert ID used in DynMoME single expert mode (Default: 0)')
parser.add_argument('--dynmome_ablation_expert_id', type=int, default=None,
                    help='Expert ID to be removed in ablation study (Default: None, means no expert removal)')
parser.add_argument('--role_encoding_type', type=str, choices=['Independent', 'Shared', 'None'], default='Independent',
                    help='Type of modality role encoding (Default: Independent)')
parser.add_argument('--top_k', type=int, default=1,
                    help='Number of experts activated in top-k routing (Default: 1)')

# Encoding Parameters
parser.add_argument('--encoding_type', type=str, choices=['BPE', 'AE'], default='AE',
                    help='Type of encoding (Default: AE)')
parser.add_argument('--encoding_pairs', type=int, default=2,
                    help='Number of encoding stack pairs (Default: 2)')
...
parser.add_argument('--dataset', type=str, choices=['gbmlgg', 'brca', 'blca', 'luad', 'ucec'], default='gbmlgg',
                    help='Dataset selection (Default: gbmlgg)')
...
if 'survival' in args.task:
    ...
    csv_path= f"./datasets_csv/tcga_{args.dataset}_all_clean_three_class.csv"
...
def main(args):
    ...
    for i in folds:
        ...
        train_dataset, val_dataset = dataset.return_splits(from_id=False,
                                                           csv_path= f"path_to_your_split_dir/tcga_{args.dataset}/splits_{i}.csv")
    ...
    if args.task_type == 'survival':
        ...
        print("Avg C-Index of {} folds: {:.3f}±{:.3f}".format(
            len(summary_all_folds), result_cindex.mean(), result_cindex.std(ddof=1)))
        ...
        with open(os.path.join(args.results_dir, 'result_summary.csv'), 'w') as f:
            for i, k in enumerate(summary_all_folds):
                ...
                print("Fold {}, Accuracy: {:.4f}, AUC: {:.4f}, AP: {:.4f}, F1: {:.4f}".format(
                    k, acc, auc, ap, f1))
                # Write results for each fold
                f.write("Fold {}, Accuracy: {:.4f}, AUC: {:.4f}, AP: {:.4f}, F1: {:.4f}\n".format(
                    k, acc, auc, ap, f1))
        ...
        print("\nAvg acc of {} folds: {:.3f}, stdp: {:.3f}, stds: {:.3f}".format(
            len(summary_all_folds), result_acc.mean(), result_acc.std(), result_acc.std(ddof=1)))
        print("\nAvg auc of {} folds: {:.3f}, stdp: {:.3f}, stds: {:.3f}".format(
            len(summary_all_folds), result_auc.mean(), result_auc.std(), result_auc.std(ddof=1)))
        print("Avg auc of {} folds: {:.3f}±{:.3f}".format(
            len(summary_all_folds), result_auc.mean(), result_auc.std(ddof=1)))
        print("Avg ap of {} folds: {:.3f}, stdp: {:.3f}, stds: {:.3f}".format(
            len(summary_all_folds), result_ap.mean(), result_ap.std(), result_ap.std(ddof=1)))
        print("Avg f1 of {} folds: {:.3f}, stdp: {:.3f}, stds: {:.3f}".format(
            len(summary_all_folds), result_f1.mean(), result_f1.std(), result_f1.std(ddof=1)))
