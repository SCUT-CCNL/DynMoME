...
def train_loop_grade_coattn_ot(epoch, bs_micro, model, loader, optimizer, n_classes, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., gc=32, args=None):
    ...
    for batch_idx, (data_WSI, data_omic1, data_omic2, data_omic3, data_omic4, data_omic5, data_omic6, label, event_time, c, grade) in enumerate(loader):
        ...
        for tindex in index_chunk_list:
            ...
            batch_loss = batch_loss + loss_micro
            
            # 累积预测结果（不参与梯度计算）
            with torch.no_grad():
                if batch_pred is None:
                    batch_pred = hazard_grade.detach()
                else:
                    batch_pred = batch_pred + hazard_grade.detach()
        ...
        with torch.no_grad():
            avg_pred = batch_pred / cnt
            pred_grade = avg_pred.argmax(dim=1, keepdim=True)
            grad_acc_epoch += pred_grade.eq(grade.view_as(pred_grade)).sum().item()
        ...
        if (batch_idx + 1) % 50 == 0:
            print('batch {}, loss: {:.4f}, grade_true: {}, event_time: {:.4f}, grade_pred: {:.4f}'.format(
                batch_idx, loss_value + loss_reg, grade.item(), float(event_time), pred_grade[0].item()))
    ...
    print('Epoch: {}, train_loss_grade: {:.4f}, train_loss: {:.4f}, acc: {:.4f}'.format(
        epoch, train_loss_grade, train_loss, acc))
...
def validate_grade_coattn_ot(cur, epoch, bs_micro, model, loader, n_classes, early_stopping=None, monitor_cindex=None, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., results_dir=None, args=None):
    ...
    for batch_idx, (data_WSI, data_omic1, data_omic2, data_omic3, data_omic4, data_omic5, data_omic6, label, event_time, c, grade) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            for tindex in index_chunk_list:
                ...
                if batch_pred is None:
                    batch_pred = hazard_grade
                else:
                    batch_pred = batch_pred + hazard_grade
            ...
            hazards_grade = soft(avg_pred)
            grade_true.append(grade)
            grade_pred.append(hazards_grade)
            
            pred_grade = hazards_grade.argmax(dim=1, keepdim=True)
            grad_acc_epoch += pred_grade.eq(grade.view_as(pred_grade)).sum().item()
            grade_pred_sum.append(pred_grade.item())
    ...
    try:
        grade_true = torch.cat(grade_true, dim=0).cpu().numpy()
        grade_pred = torch.cat(grade_pred, dim=0).cpu().numpy()
        ...
        print("Val - acc: {:.4f}, micro_auc: {:.4f}, micro_ap: {:.4f}, micro_f1: {:.4f}".format(
            acc, micro_auc, micro_ap, micro_f1))
            
    except ValueError as V:
        print("Validation metrics calculation failed:", V)
