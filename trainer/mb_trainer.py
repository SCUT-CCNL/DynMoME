...
def validate_survival_coattn_mb(cur, epoch, bs_micro, model, loader, n_classes, early_stopping=None, monitor_cindex=None, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., results_dir=None, args=None):
    ...
    CMoE_vis = None
    if args.load_model and (args.model_type == "amfm" or args.model_type == "hmfm") and args.mome_gating_network == 'CosMLP':
        print("Collecting visualization data for 4 MoME layers...")
        # Get parameters for all 4 layers
        patho1_gp, genom1_gp, patho2_gp, genom2_gp = model.get_gating_params()
        
        # Get expert selection distribution for each layer
        k_dist_P1 = patho1_gp['expert_k_counts']
        k_dist_G1 = genom1_gp['expert_k_counts']
        k_dist_P2 = patho2_gp['expert_k_counts']
        k_dist_G2 = genom2_gp['expert_k_counts']
        
        # Get expert activation distribution for each layer
        expert_dist_P1 = patho1_gp['expert_counts']
        expert_dist_G1 = genom1_gp['expert_counts']
        expert_dist_P2 = patho2_gp['expert_counts']
        expert_dist_G2 = genom2_gp['expert_counts']
        
        # Plot expert similarity matrix
        plot_expert_similarity(
            patho1_gp['sim_matrix'],
            genom1_gp['sim_matrix'],
            patho2_gp['sim_matrix'],
            genom2_gp['sim_matrix'],
            save_path=os.path.join(args.results_dir, f'expert_similarity_matrices_4layers_grade.png')
        )
        
        # Plot expert activation thresholds
        plot_expert_activation_combined(
            patho1_gp['activation_gates'],
            genom1_gp['activation_gates'],
            patho2_gp['activation_gates'],
            genom2_gp['activation_gates'],
            save_path=os.path.join(args.results_dir, f'expert_activation_gates_4layers_grade.png')
        )
        ...
        plot_expert_distribution(
            ...
            expert_dist_P2,
            expert_dist_G2,
            save_path=os.path.join(args.results_dir, f'expert_activation_distribution_4layers_grade.png')
        )
        
        # Plot expert count distribution
        plot_expert_k_distribution(
            k_dist_P1,
            k_dist_G1,
            k_dist_P2,
            k_dist_G2,
            save_path=os.path.join(args.results_dir, f'expert_activation_num_distribution_4layers_grade.png')
        )
    ...
    return patient_results, acc, micro_auc, micro_ap, micro_f1, False, CMoE_vis
...
def validate_grade_coattn(cur, epoch, model, loader, n_classes, early_stopping=None, monitor_cindex=None, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., results_dir=None, args=None):
    ...
    if early_stopping:
        ...
        if early_stopping.early_stop:
            print("Early stopping")
            return patient_results, acc, micro_auc, micro_ap, micro_f1, True,
            
    return patient_results, acc, micro_auc, micro_ap, micro_f1, False
