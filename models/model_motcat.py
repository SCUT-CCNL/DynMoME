...
class OT_Attn_assem(nn.Module):
    ...
    def forward(self,x,y):
        ...
        # only for gbmlgg：ensure both x and y are 2D
        if x.dim() == 1:
            x = x.unsqueeze(0)
        if y.dim() == 1:
            y = y.unsqueeze(0)
...
class MOTCAT_Surv(nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        ### Coattn
        #print(f"h_path_bag: {h_path_bag}")
        #print(f"h_omic_bag: {h_omic_bag}")
        A_coattn, _ = self.coattn(h_path_bag, h_omic_bag)
        A_coattn = A_coattn.squeeze()
        h_path_bag = h_path_bag.squeeze()
        if A_coattn.dim() == 1:
            A_coattn = A_coattn.unsqueeze(-1)
        if h_path_bag.dim() == 1:
            h_path_bag = h_path_bag.unsqueeze(0)
        h_path_coattn = torch.mm(A_coattn, h_path_bag).unsqueeze(1)    
